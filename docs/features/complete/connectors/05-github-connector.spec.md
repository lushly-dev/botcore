...
This spec defines the GitHub connector — the first Tier 1 concrete connector and the validation case for the connector base pattern ([01](./01-connector-base.spec.md)). It covers 8 commands under the `github_` prefix, their parameter types, output data shapes, GitHub-specific rate limit handling, pagination mapping, and default repository resolution from config. All commands return `CommandResult` values and authenticate via the GitHub auth chain ([02](./02-auth.spec.md)).
...
- The connector MUST authenticate via the GitHub auth chain defined in [02-auth.spec.md](./02-auth.spec.md)
...
Base configuration (timeout, retries, rate limiting) inherits from [01-connector-base.spec.md](./01-connector-base.spec.md).
