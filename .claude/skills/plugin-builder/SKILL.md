...
### VS Code MCP Wiring (Consumer Repo)

When the MCP server lives in a different repo than the current workspace,
add a `.vscode/mcp.json` entry in the consumer repo with:

- `"type": "stdio"`
- `"command": "uv"` and `"args": ["run", "python", "-m", "<server_module>"]`
- `"cwd"` set to the server repo (absolute path)

This avoids `No module named <package>` errors by ensuring VS Code uses the
server repo's virtual environment.
