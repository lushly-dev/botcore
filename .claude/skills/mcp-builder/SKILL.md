...
### Common Startup Failures (Lessons Learned)

- **`No module named <package>`**: VS Code is using the wrong Python env.
  Fix by using `uv run` and setting `cwd` to the repo that contains the package.
- **Server starts in the wrong repo**: If the MCP server lives in repo A but the
  workspace is repo B, set `cwd` to repo A (absolute path) in repo B's `.vscode/mcp.json`.
- **Silent failure on launch**: Missing `"type": "stdio"` in `.vscode/mcp.json`.
  VS Code does not surface a clear error in this case.
