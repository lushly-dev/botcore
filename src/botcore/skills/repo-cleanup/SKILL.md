...
source: botcore
description: >
  Periodic deep cleanup of repository-level cruft — stale branches, orphaned
  worktrees, dead test files, agent artifacts, dangling configs, and build
  output. Confirms destructive actions before executing. Use when a repo
  feels cluttered or before major milestones.
version: "1.0.0"
triggers:
  - do clean repo
  - clean repo
...
This is distinct from `commit` Step 2 (which cleans temp files before a specific commit) and `dev-environment-learn` (which covers dev tooling hygiene). This skill targets the repo itself — git state, orphaned files, and structural rot.
