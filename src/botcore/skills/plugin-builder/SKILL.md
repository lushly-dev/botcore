...
source: botcore
description: >
  Guides development of botcore plugins from protocol implementation to
  distribution. Covers BotCorePlugin protocol, PluginRegistry API, async
  command authoring with AFD/CommandResult, MCP server factory
  (create_mcp_server, meta-tool pattern), CLI generation (build_namespace),
  entry-point registration, docs registration, and skills directory bundling.
  Use when building a new botcore plugin, adding commands to a plugin,
  registering entry points, or integrating with the MCP server factory.
  Triggers: plugin, botcore plugin, BotCorePlugin, PluginRegistry, entry
  point, command authoring, CommandResult, MCP server, create_mcp_server,
  build_namespace, plugin protocol, register commands.
version: 1.1.0
triggers:
  - plugin
  - botcore plugin
...
When building a standalone bot (not a plugin) that consumes botcore packages, the
project needs additional configuration beyond basic `pip install`.

### Build System (Required)

Without a build-system, the package cannot be installed or imported by `uv` or `pip`:
...
### Local Package Sources (Required for Unpublished Deps)

Botcore sub-packages (`botcore-agents`, `botcore-llm`, `botcore-connectors`,
`botcore-memory`) are not on PyPI. Consumer projects must declare local path
references in `[tool.uv.sources]`:

```toml
[tool.uv.sources]
lushly-botcore = { path = "../botcore", editable = true }
botcore-agents = { path = "../botcore/packages/botcore-agents", editable = true }
botcore-llm = { path = "../botcore/packages/botcore-llm", editable = true }
botcore-connectors = { path = "../botcore/botcore-connectors", editable = true }
botcore-memory = { path = "../botcore/packages/botcore-memory", editable = true }
```

Without these, `uv sync` fails with "package not found in the package registry".
Adjust relative paths based on your workspace layout.

### Sync After Changes

After adding the build-system and sources, run:
...
```bash
uv run python -c "from my_package import __version__; print(__version__)"
```
