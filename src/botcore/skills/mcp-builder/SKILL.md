...
source: botcore
description: >
  Guides construction of MCP servers using FastMCP 2.x with emphasis on token
  budget management, tool consolidation, output truncation, and the execute
  pattern for Opus compatibility. Covers async subprocess patterns, safe output
  limits, and action-based tool design. Use when creating MCP servers, debugging
  tool overload or context overflow, optimizing token usage, fixing Opus silent
  failures, or consolidating tools. Triggers: MCP, FastMCP, tool schema, token
  limit, context overflow, MCP server, execute pattern, tool consolidation.
version: 1.1.0
triggers:
  - MCP
  - FastMCP
...
In `.vscode/mcp.json` (workspace-level):
...
**Critical fields:**
- `"type": "stdio"` — **required** by VS Code. Without it, the server silently fails to start.
- `"cwd"` — must point to the project that owns the venv with botcore installed.
  For consumer projects in a different repo, use an absolute path:
  ```json
  "cwd": "D:\\Github\\my-org\\my-bot"
  ```
- Use `uv run` (not bare `python`) to ensure the project's venv is activated.

For cross-workspace MCP servers (e.g., a bot defined in repo A, used from repo B),
add the server config to repo B's `.vscode/mcp.json` with `cwd` pointing to repo A.
