...
source: botcore
description: >
  Create a hotfix branch from the latest release tag, apply a minimal targeted
  fix, verify with quality gates, and fast-track through commit and PR.
  Enforces scope discipline — hotfixes are minimal fixes only, no refactoring
  or features. Delegates to commit and pr for the landing steps.
  Use for emergency or urgent production fixes.
version: "1.0.0"
triggers:
  - do hotfix
  - hotfix
...
```bash
python {skill-dir}/../commit/scripts/quality_gate.py
```
...
Delegate to `commit` in **quick mode** — quality gate already ran, so this is just the commit step.
...
Delegate to `pr` with the `hotfix` label:
...
1. If the fix should be included in an active release branch, cherry-pick:
   ...
   ```
2. Consider a patch release — delegate to `release patch` if urgent
3. Delete the hotfix branch

## Reference

See `git-manager` skill for hotfix branching strategy details.
